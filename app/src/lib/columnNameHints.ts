...
export function suggestTypeFromColumnName(columnName: string): ColumnType | null {
  ...
  if (tokens.some((t) => t === 'uuid' || t === 'guid')) return 'Uuid';
  ...
  if (tokens.some((t) => t === 'time' || t === 'duration')) return 'Time';
  if (tokens.some((t) => t === 'count' || t === 'num' || t === 'number' || t === 'id') && !tokens.includes('guid')) return 'Integer';
  if (tokens.some((t) => t === 'currency' || t === 'amount' || t === 'price' || t === 'cost' || t === 'revenue' || t === 'salary' || t === 'balance')) return 'Currency';
  if (tokens.some((t) => t === 'percent' || t === 'percentage' || t === 'pct') || tokens.includes('rate')) return 'Percentage';
